...
class DijkstrasAlorithmUsingSet
{
	...
	public static long[] shortestPath(ArrayList<ArrayList<WeightPair>> adj,int start){
	    ...
	    dist[start]=0;
	    TreeSet<Distance> set=new TreeSet<>();
	    set.add(new Distance(0,start));
	    
	    while(!set.isEmpty()){
	        
	        Distance d=set.pollFirst();
	        long dis=d.distance;
	        int curr=d.element;
	        ...
	        for(WeightPair w:adj.get(curr)){
	           if(dist[w.node] > (dis+w.weight)){
                   ...
                   set.add(new Distance(dist[w.node],w.node));
	            }
	        }
